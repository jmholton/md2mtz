...
def assign_levels(B_arr, pixel_fine, noise_frac, n_levels):
    ...
    aliasing noise target, then find the coarsest power-of-2 grid level
    whose pixel size does not exceed that requirement.

    fwhm(B) = sqrt(ln2 * (B+8)) / (2*pi)    [+8 = intrinsic B=0 width]
    pixel_req = fwhm / w_px                   [w_px from noise_wpx()]
    level L:  pixel_fine * 2^L <= pixel_req
    """
    ln2 = math.log(2.0)
    ...
    lev = np.where(ratio >= 1.0,
                   np.floor(np.log2(ratio.clip(min=1.0))).astype(np.int32),
                   0)
...
def main():
    ...
    for L, (d_L, nx_L, ny_L, nz_L) in enumerate(levels):
        ...
        print(f"    L{L}: pixel={d_L/rate/2:.2f}A  grid {nx_L}x{ny_L}x{nz_L} "
